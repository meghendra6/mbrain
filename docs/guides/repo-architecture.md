...
This is the repo MBrain indexes.
...
losing your knowledge. You can switch knowledge tools (MBrain -> something else)
...
**MBrain indexes the brain repo.** Run `mbrain sync --repo ~/brain/` to keep
the search index current. The agent repo is never indexed by MBrain.
...
3. **Don't index the agent repo.** MBrain indexes the brain repo only.
   Running `mbrain sync` against the agent repo pollutes search results
...
3. **Verify MBrain only indexes brain.** Run `mbrain stats` and check the
