...
Stop hook이 응답을 막을 때 Claude Code는 이 메시지를 `Stop hook error` 접두어 아래에 표시할 수 있습니다. 이 경우 MBrain hook이 크래시 난 것이 아니라, 세션 종료 전에 마지막 memory check를 요청하는 것입니다. 에이전트는 hook이 실행됐다는 이유만으로 쓰면 안 됩니다. 저장할 만한 durable session knowledge가 있으면 MBrain에 기록하고 sync해야 하며, 저장할 내용이 없으면 정확히 `MBRAIN-PASS: <short reason>`라고 응답하면 됩니다.
