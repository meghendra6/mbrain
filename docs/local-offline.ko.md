...
## 9. 한 명령어로 에이전트 설정: `gbrain setup-agent`
...
```bash
gbrain setup-agent              # 설치된 모든 클라이언트 자동 감지 및 설정
gbrain setup-agent --claude     # Claude Code만
gbrain setup-agent --codex      # Codex만
gbrain setup-agent --skip-mcp   # 규칙만 주입, MCP 등록 건너뜀
gbrain setup-agent --print      # 파일 쓰기 대신 stdout 출력
gbrain setup-agent --json       # 기계 판독 가능 출력
```
...
- GBrain 도구 목록 조회 요청 (`search`, `query`, `get_page` 등이 보여야 함)
...
gbrain init --local
gbrain import ~/git/brain
gbrain setup-agent               # 두 클라이언트 모두 MCP 등록 + 규칙 주입
```
