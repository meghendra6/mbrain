...
(a) writes the session's new knowledge back to gbrain, or
(b) explicitly justifies skipping (`GBRAIN-PASS: <reason>`).

## Non-goals

- Writing to gbrain mid-session or after every turn (too noisy).
...
[Session end signal]
        │
        ▼
[Stop hook: stop-gbrain-check.sh]
        │
  ┌─────┴─────┐
...
[Agent runs one more turn]
  ├─ Re-examine session for entities (per GBRAIN_AGENT_RULES §3)
  ├─ gbrain search / get existing pages
  ├─ put_page with compiled truth + timeline + back-links
  ├─ gbrain sync_brain (no_pull: true, no_embed: true)
  └─ or emit "GBRAIN-PASS: <reason>" if nothing to write
        │
        ▼
...
- The agent performs all brain writes (it has session context and gbrain MCP tools).
- The re-entry flag prevents infinite Stop-hook loops.
- `GBRAIN-PASS:` prefix is a documented protocol the hook may later inspect.

## Components

| Path | Type | Responsibility |
|---|---|---|
| `~/.claude/scripts/hooks/stop-gbrain-check.sh` | new | Stop-hook entrypoint; re-entry guard; orchestrates relevance gate and block decision |
| `~/.claude/scripts/hooks/lib/gbrain-relevance.sh` | new | Decides whether the current CWD / session warrants a brain write check |
| `~/.claude/scripts/hooks/state/gbrain-session-<id>.flag` | runtime artifact | Per-session marker that the hook already fired once |
| `~/.claude/hooks/hooks.json` | modified | Registers the new Stop hook alongside existing hooks |
| `~/.claude/logs/gbrain-stop-hook.log` | runtime artifact | Append-only log of hook decisions for observability |
...
1. If `$CLAUDE_PROJECT_DIR` or `$PWD` is listed in `~/.claude/gbrain-skip-dirs` → not relevant.
2. If environment variable `GBRAIN_STOP_HOOK=0` → not relevant (kill switch).
3. If `gbrain` CLI is not installed or not on PATH → not relevant (fail-closed in this one case to avoid surprising errors).
4. If `gbrain search --path-hint "$PWD"` (or a grep fallback over `brain/`) returns any hit within the last 30 days → relevant.
5. If CWD is registered in agent memory as a tracked project → relevant.
6. If none of the above decide → **fail-open as relevant**. Agent has the final say via `GBRAIN-PASS`.
...
| Agent's extra turn (LLM + gbrain tool calls) | 2 – 10 s (LLM-bound) |
| `put_page` + `sync_brain --no-embed --no-pull` | < 500 ms (local file + lightweight index) |
| Second Stop-hook fire (flag present) | < 5 ms |

Total perceived delay: one LLM turn beyond normal session end. gbrain operations themselves are not the bottleneck.

Embeddings are refreshed out of band via `gbrain embed --stale` (existing mechanism), not during the hook.
...
| 4 | Agent emits neither `GBRAIN-PASS` nor a write | Flag exists → second Stop passes through. Logged for later review. |
| 5 | `put_page` fails inside agent | Agent surfaces the error to the user. Not the hook's concern. |
| 6 | `sync_brain` fails | Agent warns; session still ends. `gbrain doctor` recovers later. |
...
| 9 | User wants to disable hook | `GBRAIN_STOP_HOOK=0` env var → immediate `exit 0`. |

### Logging

Append one line per invocation to `~/.claude/logs/gbrain-stop-hook.log`:
...
`test/hooks/stop-gbrain-check.bats`:
- Re-entry flag present → `exit 0`, no stdout.
- Relevance gate: CWD in skip list → `exit 0`.
- Relevance gate: `GBRAIN_STOP_HOOK=0` → `exit 0`.
- Relevance gate: `gbrain` missing on PATH → `exit 0` (documented exception).
...
`test/hooks/gbrain-relevance.bats`:
- `gbrain search --path-hint` output parsed correctly.
- Skip-dir wildcard matching.
- Fallback grep path when `--path-hint` is unsupported.

### Integration test

`scripts/test-gbrain-hook.sh`:
1. Create tmp dir, set `CLAUDE_SESSION_ID=test-<rand>`.
2. Run hook with stubbed `gbrain` binary.
...
- Read-only Q&A session ("what does `ls` do?") → observe `GBRAIN-PASS: read-only question`.
...
- Entity-detection prompt quality improvements in `GBRAIN_AGENT_RULES.md` — separate effort.
- Retroactive bootstrap of empty / stale brain pages — data-debt cleanup, separate task.
- Per-project hook scoping — user-global is intentional; `~/.claude/gbrain-skip-dirs` handles opt-outs.
