...
const list_memory_candidate_entries: Operation = {
  ...
  params: {
    scope_id: { type: 'string', description: 'Memory candidate scope id (default: workspace:default)' },
    status: {
      type: 'string',
    ...
    try {
      ...
      if (error instanceof MemoryInboxServiceError) {
        if (error.code === 'memory_candidate_not_found') {
          throw new OperationError('memory_candidate_not_found', error.message);
        }
...
export const operations: Operation[] = [
  ...
  // Profile memory
  get_profile_memory_entry, list_profile_memory_entries, upsert_profile_memory_entry, get_memory_candidate_entry, list_memory_candidate_entries, create_memory_candidate_entry, advance_memory_candidate_status, write_profile_memory_entry,
  // Personal episodes
