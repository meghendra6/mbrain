...
```bash
ls ~/.mbrain/integrations/twilio-voice-brain/heartbeat.jsonl 2>/dev/null
```
...
- Is there a public URL configured in `~/.mbrain/config.json`?
...
with `mbrain integrations show twilio-voice-brain`." Skip to Step 6.
...
`mbrain integrations show twilio-voice-brain`."
...
echo '{"version":"0.8.0","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","voice_setup":"RESULT"}' >> ~/.mbrain/migrations/completed.jsonl
```

Tell the user: "v0.8.0 migration complete. Run `mbrain integrations doctor` to
