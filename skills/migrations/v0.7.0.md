...
This migration introduces the MBrain integration recipe system. Existing users
should be informed about the new capabilities.

## What's New

MBrain now distributes **integration recipes** — markdown files that teach your
...
Phone calls create brain pages. Powered by Twilio + OpenAI Realtime + MBrain MCP.
...
### New: `mbrain integrations` Command

Discover, check status, and monitor your integrations:

```bash
mbrain integrations              # dashboard: show all senses + reflexes
mbrain integrations list --json  # list available recipes (agent-parseable)
mbrain integrations show <id>    # show recipe details + setup instructions
mbrain integrations status <id>  # check secrets + health
mbrain integrations doctor       # run health checks for all integrations
mbrain integrations stats        # signal statistics
```
...
`docs/MBRAIN_SKILLPACK.md` still works (backward compatible).

## Agent Instructions

When presenting this update to the user, say something like:

"MBrain v0.7.0 adds integration recipes — your brain can now get new senses
automatically. The first one is Voice-to-Brain: phone calls create brain pages.
Want me to show you what's available? Run `mbrain integrations` to see the
dashboard."

If the user is interested in voice-to-brain:
1. Run `mbrain integrations show twilio-voice-brain` to see what's needed
