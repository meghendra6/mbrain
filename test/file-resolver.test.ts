...
describe('file-resolver', () => {
  ...
  test('blocks .supabase marker with traversal prefix', async () => {
    ...
    await expect(resolveFile('poisoned/secret.json', brainRoot, storage)).rejects.toThrow('marker prefix contains path traversal');
  });
  ...
  test('blocks .supabase marker with absolute path prefix', async () => {
    ...
    await expect(resolveFile('abs/passwd', brainRoot, storage)).rejects.toThrow('marker prefix contains path traversal');
  });
